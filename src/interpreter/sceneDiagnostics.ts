...
const HELPER_NODE_NAMES = new Set([
    "Character",
    "CharacterPose",
    "ControlSet",
    "ControlSetPlug",
...
export function extractSceneDiagnostics(objectMap: FBXObjectMap): FBXSceneDiagnostic[] {
    ...
    for (const [id, node] of objectMap.objects) {
        ...
        if (node.name === "Constraint") {
            diagnostics.push(createObjectDiagnostic(
                objectMap,
                id,
                node,
                "unsupported-constraint",
        ...
        if (HELPER_NODE_NAMES.has(node.name)) {
            diagnostics.push(createObjectDiagnostic(
                ...
                node,
                "unsupported-helper",
                `${node.name} helper data is preserved as diagnostic data but not evaluated at runtime.`
            ));
            continue;
        }
        ...
        if (node.name === "Pose" && subType !== "BindPose") {
            diagnostics.push(createObjectDiagnostic(
                ...
                node,
                "unsupported-pose",
                `Pose subtype '${subType}' is preserved as diagnostic data but not evaluated at runtime.`
            ));
            continue;
        }
        ...
        if (node.name === "Deformer" && !isSupportedDeformer(subType)) {
            diagnostics.push(createObjectDiagnostic(
                ...
                node,
                "unsupported-deformer",
                `Deformer subtype '${subType}' is preserved as diagnostic data but not evaluated at runtime.`
            ));
            continue;
        }
...
function createObjectDiagnostic(

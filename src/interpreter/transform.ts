...
export function computeFBXGeometricMatrix(
    translation: FBXVector3,
    rotation: FBXVector3,
    scale: FBXVector3
): Matrix {
...
export function computeFBXGeometricDeltaMatrix(
    rotation: FBXVector3,
    scale: FBXVector3
): Matrix {
...
export function computeFBXGeometricNormalMatrix(
    rotation: FBXVector3,
    scale: FBXVector3
): Matrix {
    const d2r = Math.PI / 180;
...
export function computeFBXLocalMatrix(components: FBXTransformComponents): Matrix {
    ...
    if (!hasPivots && !hasOffsets && !hasPostRot) {
        const preRotM = eulerToMatrixXYZ(
            preRotation[0] * d2r, preRotation[1] * d2r, preRotation[2] * d2r
        );
        const lclRotM = eulerToMatrix(
    ...
    const Rpre = eulerToMatrixXYZ(
        preRotation[0] * d2r, preRotation[1] * d2r, preRotation[2] * d2r
    );
    const R = eulerToMatrix(
        rotation[0] * d2r, rotation[1] * d2r, rotation[2] * d2r, rotationOrder
    );

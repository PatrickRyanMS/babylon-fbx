import type { FBXDocument, FBXNode, FBXPropertyValue } from "../types/fbxTypes.js";
...
export function extractPropertyTemplates(doc: FBXDocument): FBXPropertyTemplateMap {
    ...
    for (const objectTypeNode of definitions.children) {
        ...
        for (const templateNode of objectTypeNode.children) {
            if (templateNode.name !== "PropertyTemplate") continue;
...
export function getPropertyTemplate(
    ...
    objectType: string,
    templateName?: string
): FBXPropertyTemplate | undefined {
    const templatesByName = templates.get(objectType);
    if (!templatesByName) return undefined;
    if (templateName) return templatesByName.get(templateName);
    return templatesByName.values().next().value;
}

export function getTemplatePropertyValue<T extends FBXPropertyValue>(
...
export function resolvePropertyValue<T extends FBXPropertyValue>(
    node: FBXNode,
    template: FBXPropertyTemplate | undefined,
    propertyName: string,
    valueIndex = 0
...
export function resolveNumberProperty(
    node: FBXNode,
    template: FBXPropertyTemplate | undefined,
    propertyName: string,
    fallback: number
...
export function resolveVector2Property(
    node: FBXNode,
    template: FBXPropertyTemplate | undefined,
    propertyName: string,
...
export function resolvePropertyValues(
    ...
    template: FBXPropertyTemplate | undefined,
    propertyName: string
): FBXPropertyValue[] | undefined {
    return findLocalPropertyValues(node, propertyName)
        ?? template?.properties.get(propertyName)?.values;
}
...
function extractPropertyTemplate(
    objectType: string,
    templateName: string,
    templateNode: FBXNode
): FBXPropertyTemplate {
    ...
    for (const propertyNode of properties70?.children ?? []) {
        if (propertyNode.name !== "P") continue;
...
function findLocalPropertyValues(node: FBXNode, propertyName: string): FBXPropertyValue[] | undefined {
    const propertyContainers = [
        findChildByName(node, "Properties70"),
        findChildByName(node, "Properties60"),
    ].filter((child): child is FBXNode => child !== undefined);
    ...
    for (const container of propertyContainers) {
        for (const propertyNode of container.children) {
            ...
            if (getPropertyValue<string>(propertyNode, 0) !== propertyName) continue;
            return propertyNode.properties
                .slice(propertyNode.name === "Property" ? 3 : 4)

...
export function extractGeometry(geometryNode: FBXNode, nodeId: bigint): FBXGeometryData {
    ...
    const binormals = binormalNode
        ? expandLayerElement(binormalNode, "Binormals", "BinormalsIndex", polyVertexList, rawPositions.length / 3, 3, diagnostics)
        : null;
    const tangents = tangentNode
        ? expandTangentLayer(tangentNode, polyVertexList, rawPositions.length / 3, normals, binormals, diagnostics)
        : null;
...
function triangulatePolygon(
    ...
    while (remaining.length > 3 && guard++ < poly.indices.length * poly.indices.length) {
        ...
        for (let i = 0; i < remaining.length; i++) {
            ...
            if (!isConvex(projected[prev], projected[curr], projected[next], isCCW)) continue;
            if (containsAnyPoint(projected, remaining, prev, curr, next)) continue;
    ...
    clipped.push({
        vertices: [
            poly.startIndex + remaining[0],
            poly.startIndex + remaining[1],
            poly.startIndex + remaining[2],
        ],
        polyIndex,
    });
    return clipped;
...
function projectPolygonTo2D(poly: Polygon, rawPositions: Float64Array): [number, number][] | null {
    ...
    return poly.indices.map((cp) => {
        ...
        if (dropAxis === 0) return normal[0] >= 0 ? [y, z] : [z, y];
        if (dropAxis === 1) return normal[1] >= 0 ? [z, x] : [x, z];
...
function expandTangentLayer(
    ...
    for (let i = 0; i < polyVertexList.length; i++) {
        ...
        tangents[destOffset + 3] = sourceStride >= 4
            ? expanded[sourceOffset + 3]
            : computeTangentHandedness(i, tangents, normals, binormals);
    }
    return tangents;
}
...
function inferLayerElementStride(
    ...
    if (directCount > 0 && data.length % directCount === 0) {
        ...
        if (stride === 3 || stride === 4) return stride;
    }

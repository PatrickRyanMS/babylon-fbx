...
export function resolveRigs(objectMap: FBXObjectMap, skins: FBXSkinData[]): FBXRigData[] {
    ...
    for (const skin of skins) {
        const clusterModelIds = skin.bones
            .filter((bone) => bone.isCluster)
            .map((bone) => bone.modelId);
        if (clusterModelIds.length === 0) continue;
...
function buildRig(rootModelId: bigint, skins: FBXSkinData[], objectMap: FBXObjectMap): FBXRigData {
    ...
    for (const [modelId, sources] of sourceBonesByModelId) {
    ...
    for (const modelId of orderedModelIds) {
        ...
        const parentIndex = parentModelId === undefined
            ? -1
            : modelIdToBoneIndex.get(parentModelId) ?? -1;
    ...
    const skinBindings = skins.map((skin) =>
        buildSkinBinding(skin, `rig_${rootModelId.toString()}`, modelIdToBoneIndex)
    );
...
function orderParentsBeforeChildren(
    modelIds: Set<bigint>,
    parentByModelId: Map<bigint, bigint>,
    sourceOrderByModelId: Map<bigint, number>
): bigint[] {
    ...
    for (const children of childrenByModelId.values()) {
...
function collectTransformLinkWarnings(sourceBonesByModelId: Map<bigint, FBXBoneData[]>): string[] {
    ...
    for (const [modelId, sources] of sourceBonesByModelId) {
        const matrices = sources
            .filter((bone) => bone.isCluster && bone.transformLinkMatrix)
            .map((bone) => bone.transformLinkMatrix!);
        if (matrices.length < 2) continue;
...
function createFallbackBone(modelId: bigint, objectMap: FBXObjectMap): FBXBoneData | null {
    ...
    if (!modelNode || modelNode.name !== "Model") return null;

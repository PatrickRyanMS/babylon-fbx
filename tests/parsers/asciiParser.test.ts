...
const FIXTURE_PATH = resolve(__dirname, "../models/valkyrie/valkyrie_asset_ascii.fbx");
...
describe("parseAsciiFBX", () => {
    ...
    it("should parse PolygonVertexIndex as int32-like array", () => {
        const objects = findDocumentNode(doc, "Objects");
        const geometry = objects!.children.find((c) => c.name === "Geometry");
        const pvi = geometry!.children.find((c) => c.name === "PolygonVertexIndex");
        expect(pvi).toBeDefined();
        const indices = pvi!.properties[0].value as Float64Array;

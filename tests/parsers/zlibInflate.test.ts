...
import { inflateZlib } from "../../src/parsers/zlibInflate.js";
...
describe("inflateZlib", () => {
    ...
    it("inflates a fixed-Huffman ASCII payload", () => {
        const output = inflateZlib(
            hex("78 9c f3 48 cd c9 c9 d7 51 08 cf 2f ca 49 51 04 00 1f 9e 04 6a"),
            13
        );
    ...
    it("inflates a real compressed FBX array payload through the binary parser", () => {
        const buffer = readFileSync(resolve(__dirname, "../models/valkyrie/valkyrie_asset.fbx"));
        const doc = parseBinaryFBX(buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength));
        const geometry = findDocumentNode(doc, "Objects")!.children.find((node) => node.name === "Geometry")!;
        const vertices = geometry.children.find((node) => node.name === "Vertices")!;

        expect(vertices.properties[0].type).toBe("float64[]");
        expect((vertices.properties[0].value as Float64Array).length).toBeGreaterThan(0);
    });

    it("inflates compressed FBX boolean arrays as one byte per element", () => {
        const buffer = readFileSync(resolve(__dirname, "../models/mech-drone/Drone.FBX"));
        const doc = parseBinaryFBX(buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength));
        const geometry = findDocumentNode(doc, "Objects")!.children.find((node) => node.name === "Geometry")!;
        const visibilityLayer = geometry.children.find((node) => node.name === "LayerElementVisibility")!;
        const visibility = visibilityLayer.children.find((node) => node.name === "Visibility")!;

        expect(visibility.properties[0].type).toBe("boolean[]");
        expect(visibility.properties[0].value).toBeInstanceOf(Uint8Array);
        expect((visibility.properties[0].value as Uint8Array).length).toBe(14132);
    });

    it("rejects truncated input", () => {
        const compressed = hex("78 9c f3 48 cd c9 c9 d7 51 08 cf 2f ca 49 51 04 00 1f 9e 04 6a");

        expect(() => inflateZlib(compressed.subarray(0, compressed.byteLength - 1), 13)).toThrow(
            /unexpected end|adler32 mismatch|trailing deflate data/
        );
    });
    ...
    it("rejects invalid stored-block lengths", () => {
        ...
        expect(() => inflateZlib(invalidStoredBlock, 5)).toThrow(
    ...
    it("does not depend on pako", () => {
        const packageJson = readFileSync(resolve(__dirname, "../../package.json"), "utf8");
        const parser = readFileSync(resolve(__dirname, "../../src/parsers/fbxBinaryParser.ts"), "utf8");

        expect(packageJson).not.toContain("pako");
        expect(parser).not.toContain("pako");
    });
});
